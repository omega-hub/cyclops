...
/* Compositor - XML parsing methods */

osg::Camera* Compositor::createPassFromXML( osgDB::XmlNode* xmlNode )
{
    std::string name = xmlNode->properties["name"];
    if ( name.empty() )
        owarn("Compositor: inappropriate to create pass with empty name");
    ...
    for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
    {
        ...
        if ( childName=="output_buffer" )
        {
            ...
            if ( xmlChild->children.size()>0 )
            {
                ...
                numAttached++;
                ofwarn("Compositor: <pass> %1% is outputting to a local buffer which cannot be shared anymore", %name);
            }
            else
            {
                ...
                if ( texture )
                {
                    ...
                    ofwarn("Compositor: <pass> can't find global texture object %1%", %xmlChild->getTrimmedContents());
            }
        }
        else if ( childName=="texture" || childName=="input_buffer" )
        {
            ...
            if ( xmlChild->children.size()>0 )
            {
                ...
                if ( childName=="input_buffer" )
                    ofwarn("Compositor: <pass> %1% is using a local buffer which cannot be attached by others", %name);
            }
            else
            {
                ...
                else ofwarn("Compositor: <pass> can't find global texture object %1%", %xmlChild->getTrimmedContents());
            }
        }
        else if ( childName=="uniform" )
        {
            ...
            else
            {
                ...
                else ofwarn("Compositor: <pass> can't find global uniform object %1%", %xmlChild->getTrimmedContents());
            }
        }
        else if ( childName=="shader" )
        {
            ...
            else
            {
                ...
                else ofwarn("Compositor: <pass> can't find global shader object %1%", %xmlChild->getTrimmedContents());
            }
        }
        else if ( childName=="vertex_attribute" )
        {
            ...
            if ( !attrIndex.empty() )
                ...
                ofwarn("Compositor: <vertex_attribute> %1% doesn't have a valid index", %attrName);
        }
        ...
        else if ( childName=="near_far" )
        {
            ...
            ofwarn("Compositor: <pass> doesn't recognize child element %1%", %xmlChild->name);
    }
...
osg::Texture* Compositor::createTextureFromXML( osgDB::XmlNode* xmlNode, bool asGlobal )
{
    osg::ref_ptr<osg::Texture> texture;
    std::string name = xmlNode->properties["name"];
    std::string type = xmlNode->properties["type"];
    if ( name.empty() )
        owarn("Compositor: inappropriate to create texture/buffer with empty name");
    ...
    for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
    {
        ...
        if ( childName=="file" )
        {
            ...
            if ( !image )
            {
                ofwarn("Compositor: <texture> failed to load <file>: %1%",
                           %xmlChild->getTrimmedContents());
            }
        ...
        else if ( childName=="rawfile" )
        {
            ...
            {
                ofwarn("Compositor: <texture> failed to load <rawfile>: %1%",
                           %rawfile);
            }
        }
        ...
        else if ( childName=="internal_format" )
        {
            ...
            else
            {
				...
				// need to print in hex..
                ofwarn("Compositor: <texture> try to use internal format: %1%",
                           %atoi(format.c_str()));
            }
        ...
        else if ( childName=="source_format" )
        {
            ...
            else
            {
                ...
                ofwarn("Compositor: <texture> try to use source format: %1%",
                           %atoi(format.c_str()));
            }
        }
        else if ( childName=="source_type" )
        {
            ...
            else
            {
                ...
                ofwarn("Compositor: <texture> try to use data type: %1%",
                           %atoi(dataType.c_str()));
            }
        }
        else
            ofwarn("Compositor: <texture> doesn't recognize child element %1%", %xmlChild->name);
    }
    ...
    if ( asGlobal )
    {
        if ( !setTexture(name, texture.get()) )
            ofwarn("Compositor: <texture> object name %1% already exists", %name);
        return texture.get();
    }
    else
...
osg::Uniform* Compositor::createUniformFromXML( osgDB::XmlNode* xmlNode, bool asGlobal )
{
    std::string name = xmlNode->properties["name"];
    if ( name.empty() )
        owarn("Compositor: inappropriate to create uniform with empty name");
    ...
    for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
    {
        ...
        if ( childName=="value" )
        {
            ...
            case GL_UNSIGNED_INT:
                ...
                break;
            default:
                ofwarn("Compositor: <uniform> %1% doesn't have a recognizable value type: %2%", %name %typeString);
                break;
            }
            
            if ( ss.rdstate()&ss.failbit )
                ofwarn("Compositor: <uniform> %1% doesn't have a recognizable value: %2%", %name %xmlChild->getTrimmedContents());
            uniform->dirty();
        }
        else if ( childName=="inbuilt_value" )
        {
            ...
            // These are defined in the Compositor:
            ...
            {
                ofwarn("Compositor: <inbuilt_value> of %1% doesn't have a recognizable value: %2%", %name %valueName);
            }
        }
        else if ( childName=="animation" )
        {
            ...
            if ( unsupported )
            {
                ofwarn("Compositor: <animation> of %1% can't support the data type %2%", %name %typeString);
                continue;
            }
            ...
            ofwarn("Compositor: <uniform> doesn't recognize child element %1%", %xmlChild->name);
    }
    
    if ( asGlobal )
    {
        if ( !setUniform(name, uniform.get()) )
            ofwarn("Compositor: <uniform> object name %1% already exists", %name);
        return uniform.get();
    }
    else
...
osg::Shader* Compositor::createShaderFromXML( osgDB::XmlNode* xmlNode, bool asGlobal )
{
    osg::ref_ptr<osg::Shader> shader;
    std::string name = xmlNode->properties["name"];
    if ( name.empty() )
        owarn("Compositor: inappropriate to create shader with empty name");
    ...
    for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
    {
        ...
        if ( childName=="source" )
        {
            if ( xmlChild->children.size()>0 )
            {
                if ( xmlChild->children[0]->type==osgDB::XmlNode::INFORMATION )
                    ...
                    ofwarn("Compositor: <shader> %1% doesn't have a valid <source> definition", %name);
            }
            else
            {
                ofwarn("Compositor: <shader> %1% specifies the source without using CDATA", %name);
        ...
        else if ( childName=="file" )
        {
            ...
            if ( shaderFile.empty() )
            {
                ofwarn("Compositor: <shader> failed to load <file>: %1%",
                           %xmlChild->getTrimmedContents());
            }
            else
            {
            ...
            ofwarn("Compositor: <shader> doesn't recognize child element %1%", %xmlChild->name);
    }
    ...
    if ( asGlobal )
    {
        if ( !setShader(name, shader.get()) )
            ofwarn("Compositor: <shader> object name %1% already exists", %name);
        return shader.get();
    }
    else
...
bool Compositor::loadFromXML( osgDB::XmlNode* xmlNode, XmlTemplateMap& templateMap, const osgDB::Options* options )
{
    ...
    for ( unsigned int i=0; i<xmlNode->children.size(); ++i )
    {
        ...
        else
        {
            ...
            if ( !ref.empty() )
            {
                ...
                else ofwarn("Compositor: <template> %1% not found while applying to %2%", %ref %xmlChild->name);
            }
        }
        ...
        if ( childName=="technique" )
        {
            ...
            for ( unsigned int j=0; j<xmlChild->children.size(); ++j )
            {
                ...
                if ( !ref.empty() )
                {
                    ...
                    else ofwarn("Compositor: <template> %1% not found while applying to ", %ref %xmlPassChild->name);
                }
                
                if ( xmlPassChild->name.find("pass")!=std::string::npos )
                    createPassFromXML( xmlPassChild );
                else
                    ofwarn("Compositor: <technique> doesn't recognize child element %1%", %xmlPassChild->name);
            }
        }
            ...
            ofwarn("Compositor: doesn't recognize global element %1%", %childName);
    }
    return true;
}
...
Compositor* cyclops::readEffectFile( const std::string& filename, const osgDB::Options* options )
{
	String fullpath;
	if(DataManager::findFile(filename, fullpath))
	{
		osg::ref_ptr<osgDB::XmlNode> xmlRoot = osgDB::readXmlFile( fullpath, options );
		if ( xmlRoot.valid() )
		{
			...
			Compositor::XmlTemplateMap templateMap;
	...
	{
		ofwarn("could not fild compositor definition file %1%", %filename);
	}

    return NULL;
}

Compositor* cyclops::readEffectStream( std::istream& stream, const osgDB::Options* options )
{
