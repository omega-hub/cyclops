...
{
public:
    PassCullCallback( Compositor* compositor, Compositor::PassType type )
    ...
    virtual void operator()( osg::Node* node, osg::NodeVisitor* nv )
    {
        ...
        if ( nv->getVisitorType()==osg::NodeVisitor::CULL_VISITOR )
        {
            ...
            if ( _type==Compositor::FORWARD_PASS )
            {
...
protected:
    osg::observer_ptr<Compositor> _compositor;
    Compositor::PassType _type;
};
...
Compositor::Compositor( const Compositor& copy, const osg::CopyOp& copyop )
...
osg::Camera* Compositor::createNewPass( PassType type, const std::string& name )
{
...
bool Compositor::removePass( const std::string& name )
{
...
bool Compositor::getPassData( const std::string& name, PassData& data ) const
{
...
bool Compositor::setPassIndex( const std::string& name, unsigned int index )
{
...
unsigned int Compositor::getPassIndex( const std::string& name ) const
{
    const PassList& passList = getPassList();
    for ( unsigned int i=0; i<passList.size(); ++i )
    {
...
bool Compositor::setPassActivated( const std::string& name, bool activated )
{
    PassList& passList = getPassList();
    for ( unsigned int i=0; i<passList.size(); ++i )
    {
...
bool Compositor::getPassActivated( const std::string& name ) const
{
    const PassList& passList = getPassList();
    for ( unsigned int i=0; i<passList.size(); ++i )
    {
...
const Compositor::PassList& Compositor::getPassList() const
{
    PassListMap::const_iterator itr = _passLists.find( _currentTechnique );
    if ( itr==_passLists.end() )
    {
        static PassList s_emptyPassList;
        ofwarn("Not a valid technique name: %1%", %_currentTechnique);
        return s_emptyPassList;
    }
    return itr->second;
}

osg::NodeList Compositor::getCameras( Compositor::PassType type ) const
{
...
bool Compositor::setTexture( const std::string& name, osg::Texture* tex )
{
...
bool Compositor::removeTexture( const std::string& name )
{
    TextureMap::iterator itr = _textureMap.find(name);
...
osg::Texture* Compositor::getTexture( const std::string& name )
{
    TextureMap::iterator itr = _textureMap.find(name);
...
const osg::Texture* Compositor::getTexture( const std::string& name ) const
{
...
bool Compositor::setUniform( const std::string& name, osg::Uniform* uniform )
{
...
bool Compositor::removeUniform( const std::string& name )
{
    UniformMap::iterator itr = _uniformMap.find(name);
...
osg::Uniform* Compositor::getUniform( const std::string& name )
{
    UniformMap::iterator itr = _uniformMap.find(name);
...
const osg::Uniform* Compositor::getUniform( const std::string& name ) const
{
...
void Compositor::removeInbuiltUniform( InbuiltUniformType t )
{
...
void Compositor::removeInbuiltUniform( osg::Uniform* u )
{
    for ( InbuiltUniformList::iterator itr=_inbuiltUniforms.begin(); itr!=_inbuiltUniforms.end(); )
    {
        if ( itr->second==u )
...
bool Compositor::isInbuiltUniform( osg::Uniform* u ) const
{
...
bool Compositor::setShader( const std::string& name, osg::Shader* shader )
{
...
bool Compositor::removeShader( const std::string& name )
{
    ShaderMap::iterator itr = _shaderMap.find(name);
...
osg::Shader* Compositor::getShader( const std::string& name )
{
...
const osg::Shader* Compositor::getShader( const std::string& name ) const
{
    ShaderMap::const_iterator itr = _shaderMap.find(name);
    if ( itr==_shaderMap.end() ) return NULL;
    else return itr->second.get();
...
osg::Geode* Compositor::getOrCreateQuad()
{
    if ( !_quad )
    {
...
void Compositor::setPreservedNearAndFar( unsigned int frame, double zn, double zf )
{
...
osg::Geode* Compositor::createScreenQuad( float width, float height, float scale )
{
    osg::Geometry* geom = osg::createTexturedQuadGeometry(
